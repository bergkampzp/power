...
# ---- API 注册 ----
APIS = [
    ("日前电价(全省)", lambda d: api_post("tdSpotRecentlyResultUserInfo/getList", {"operatingDate": d}),
     lambda c,d,r: insert_day_ahead_price(c,d, r.get("data",{}).get("data", r.get("data",[])))),
    ("统调负荷", lambda d: api_post("intePublishDayAutotuneCurve/getObj", {"exchange":"04","runTime": d.replace("-","")}),
     lambda c,d,r: insert_load(c,d, r.get("data",{}).get("data",[]))),
    ("发电总出力", lambda d: api_post("mkGenElecTotalPrediction/getObject", {"runTime": d.replace("-","")}),
     lambda c,d,r: insert_gen_total(c,d, r.get("data",{}).get("data",[]))),
    ("非市场出力", lambda d: api_post("intePublishNonmarketUnitCurve/getList", {"exchange":"04","runTime": d.replace("-","")}),
     lambda c,d,r: insert_nonmarket(c,d, r.get("data",{}).get("data",[]))),
    ("非市场不含新能源", lambda d: api_post("mkXhNonMarketEleNewPowerPre/getList", {"runTime": d.replace("-",""),"areaCode":"04"}),
     lambda c,d,r: insert_nonmarket_no_renew(c,d, r.get("data",{}).get("data",[]))),
    ("新能源出力", lambda d: api_post("intePublishNewEnergyDay/getList", {"runTime": d.replace("-",""),"exchange":"04","dayType":"2","dataType":"1"}),
     lambda c,d,r: insert_renewable(c,d, r.get("data",{}).get("data",[]))),
    ("水电预测", lambda d: api_post("mkWaterPowerWeekPredOutput/getListPage", {"pageNum":1,"pageSize":50,"runTime": d.replace("-",""),"areaName":"云南"}),
     lambda c,d,r: insert_hydro(c,d, r.get("data",{}).get("data",{}).get("list",[]))),
    ("水电出力96点", lambda d: api_post("mkWaterElecTicTotalOutput/getObject", {"runTime": d.replace("-",""),"areaNo":"04"}),
     lambda c,d,r: insert_hydro_actual(c,d, r.get("data",{}).get("data",[]))),
    ("实时+出力", lambda d: api_post("tdSpotRealResultNodeInfo/getList", {"operatingDate": d}),
     lambda c,d,r: insert_real_result(c,d, r.get("data",{}).get("data",[]))),
    ("负荷(实际)", lambda d: api_post("intePublishVolumeUpCurve/getListPage", {"exchange":"04","runTime": d.replace("-","")}),
     lambda c,d,r: insert_load2(c,d, r.get("data",{}).get("data",{}).get("list",[]))),
]

def sync_date(conn, date_str):
    print(f"\n📅 {date_str}")
    day_result = {}
    for name, api_fn, insert_fn in APIS:
        try:
...
def sync_range(start_date, end_date, batch_size=21):
    ...
    while current <= end:
        ...
        all_results[ds] = sync_date(conn, ds)
...
if __name__ == "__main__":
    if len(sys.argv) > 2:
        ...
        gap_analysis()
        print("\n要同步所有数据，运行：")
        print("  python3 sync_all.py 2026-03-22 2026-06-05")
